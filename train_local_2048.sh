...
find "${dataset_path}" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.bmp" -o -iname "*.gif" \) -print0 | while IFS= read -r -d '' img_path; do
    ...
    if [ ! -f "/home/xi9/code/FastDiffVG/output/fast_diffvg_area_2048/${dataset}/$(printf "%04d" ${counter})/final.png" ]; then
        echo "output directories do not exist for $Basename. Skipping job submission.  /home/xi9/code/FastDiffVG/output/fast_diffvg_area_2048/${dataset}/$(printf "%04d" ${counter})/final.png"
